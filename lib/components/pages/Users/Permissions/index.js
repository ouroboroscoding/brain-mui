...
export default function Permissions({ ids, onClose, onUpdate, portals, value }) {
    ...
    function change(name, val) {
        ...
        if (empty(val)) {
            delete dPermissions[tabs[tab]][name];
        }
        // Else, update / add the rights
        else {
            dPermissions[tabs[tab]][name] = val;
        }
        // Update the state
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Tabs, { onChange: (ev, i) => {
            ...
            omap(permissions, ((o, p) => React.createElement(Tab, { key: p, label: portals[p].title }))),
            !empty(remaining) &&
        ...
        tab !== false && portals[tabs[tab]].permissions.map(section => React.createElement(Paper, { key: section.title, className: "permissions" },
